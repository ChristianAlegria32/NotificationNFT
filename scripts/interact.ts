...
async function main() {
    ...
    const nftCustodyAddress = "0xB9E5ec8AF291F7247b36129866A56374A647b7AF"; // Use your actual NFTCustody contract address
    ...
    try {
        ...
        await NFTCustody.connect(deployer).grantRole(MINTER_ROLE, deployer.address, { gasLimit: 1000000 });
        console.log("Granted MINTER_ROLE to deployer");

        await NFTCustody.connect(deployer).grantRole(MINTER_ROLE, user1.address, { gasLimit: 1000000 });
        console.log("Granted MINTER_ROLE to user1");

        // Mint an NFT and set metadata with manual gas limit
        await NFTCustody.connect(user1).depositNFT("user1", 1, NFTCustody.address, { gasLimit: 1000000 });
        console.log("NFT deposited");

        await NFTCustody.connect(user1).claimNFT("user1", user1.address, "Forward", "Example Team", 100, "https://example.com/1.png", { gasLimit: 1000000 });
